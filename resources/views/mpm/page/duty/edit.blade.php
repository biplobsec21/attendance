    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-8 px-4">
        <div class="container mx-auto max-w-2xl">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Duty Record</h1>
                <p class="text-gray-600">Update the details for this duty assignment</p>
            </div>

            <!-- Main Form Card -->
            <div class="bg-white/80 backdrop-blur-sm shadow-xl rounded-3xl border border-white/50 overflow-hidden">
                <div class="p-8">
                    {{-- The form now points to the 'duty.update' route using the PUT method --}}
                    <form method="POST" action="{{ route('duty.update', $duty->id) }}" id="duty-form">
                        {{-- CSRF token and method spoofing for PUT --}}
                        @csrf
                        @method('PUT')
                                                    ...
                                                    @for ($m = 0; $m < 60; $m += 30)
                                                        @php
                                                            ...
                                                            $selected =
                                                                old(
                                                                    'start_time',
                                                                    \Carbon\Carbon::parse($duty->start_time)->format(
                                                                        'H:i',
                                                                    ),
                                                                ) == $time
                                                                    ? 'selected'
                                                    ...
                                                    @for ($m = 0; $m < 60; $m += 30)
                                                        @php
                                                            ...
                                                            $selected =
                                                                old(
                                                                    'end_time',
                                                                    \Carbon\Carbon::parse($duty->end_time)->format(
                                                                        'H:i',
                                                                    ),
                                                                ) == $time
                                                                    ? 'selected'
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Edit script loaded successfully');
            ...
            // Initialize with existing duty ranks
            @foreach ($duty->dutyRanks as $dutyRank)
                selectedRanks[{{ $dutyRank->rank_id }}] = {
                    id: {{ $dutyRank->rank_id }},
                    name: "{{ $dutyRank->rank->name }}",
            ...
            // Initialize the dropdown options based on existing selections
            rankOptions.forEach(option => {
                ...
                // If this rank is already selected, mark it as such
            ...
            // Initialize the display with existing data
            updateSelectedRanksDisplay();
