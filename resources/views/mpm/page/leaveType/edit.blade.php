...
@section('content')
    ...
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white/30 shadow-lg rounded-lg p-6 formBack w-full max-w-lg mx-auto">
            <h1 class="text-2xl font-bold mb-6 text-center text-gray-800">Edit LeaveType</h1>
            ...
            <form method="POST" action="{{ route('leave-types.update', $leaveType) }}">
                ...
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2">
                        Leave Type Name <span class="text-red-600">*</span>
                    </label>
                    <input type="text" id="name" name="name" value="{{ old('name', $leaveType->name) }}"
                        ...
                        placeholder="Enter LeaveType name" required>
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <select id="status" name="status"
                        ...
                        required>
                        <option value="1" {{ old('status', $leaveType->status ? '1' : '0') == '1' ? 'selected' : '' }}>
                            Active
                        </option>
                        <option value="0" {{ old('status', $leaveType->status ? '1' : '0') == '0' ? 'selected' : '' }}>
                            Inactive</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
