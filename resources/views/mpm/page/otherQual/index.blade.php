...
@section('title', 'Other Qualifications List')

@section('content')
<div class="container mx-auto p-4">
    <div class="bg-white/30 shadow-lg rounded-lg p-4 sm:p-6 formBack">
        <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <h1 class="text-2xl font-bold text-gray-800">Qualifications List</h1>
            <div class="w-full md:w-auto flex flex-col sm:flex-row items-center gap-2">
                ...
                <input type="text" id="table-search" class="w-full sm:w-auto px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Search qualifications...">
                <a href="{{ url('otherQual/create') }}" class="w-full sm:w-auto bg-transparent text-black font-semibold py-2 px-4 border border-black rounded-lg hover:bg-black hover:text-white transition-colors text-center no-underline">Add New Qualification</a>
            </div>
        </div>
            ...
            <table class="min-w-full bg-transparent rounded-lg">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Qualification Name</th>
                ...
                <tbody id="qual-table-body" class="divide-y divide-gray-300">
...
<script>
    // --- Qualification Data ---
    const qualData = [
        { sl: 1, name: "First Aid Certified", remark: "Completed a certified first aid course.", status: "Active" },
        { sl: 2, name: "Scuba Certified", remark: "PADI Open Water Diver.", status: "Active" },
        { sl: 3, name: "Public Speaking", remark: "Toastmasters International.", status: "Active" },
        { sl: 4, name: "Project Management", remark: "Expired certification.", status: "Inactive" },
        { sl: 5, name: "Fluent in Spanish", remark: "Advanced level proficiency.", status: "Active" }
    ];

    // --- Table Search and Pagination Logic ---
    const tableBody = document.getElementById('qual-table-body');
    const searchInput = document.getElementById('table-search');
    const paginationControls = document.getElementById('pagination-controls');
    const rowsPerPageSelect = document.getElementById('rows-per-page');
    
    let currentPage = 1;
    let rowsPerPage = rowsPerPageSelect.value === 'all' ? qualData.length : parseInt(rowsPerPageSelect.value);
    let filteredData = [...qualData];
    ...
    function toggleStatus(qualId) {
        const qual = qualData.find(item => item.sl === qualId);
        if (qual) {
            qual.status = qual.status === 'Active' ? 'Inactive' : 'Active';
        }
        const filteredQual = filteredData.find(item => item.sl === qualId);
        if (filteredQual) {
            filteredQual.status = qual.status;
        }
        displayTable(currentPage);
    }
    ...
    tableBody.addEventListener('click', (e) => {
        if (e.target.matches('span[data-id]')) {
            const qualId = parseInt(e.target.dataset.id);
            toggleStatus(qualId);
        }
    });
    ...
    searchInput.addEventListener('keyup', (e) => {
        ...
        filteredData = qualData.filter(item => {
    ...
    rowsPerPageSelect.addEventListener('change', (e) => {
        ...
        if (selectedValue === 'all') {
            rowsPerPage = filteredData.length > 0 ? filteredData.length : qualData.length;
        } else {
