...
<div class="container mx-auto p-4">
    <div class="bg-white/30 shadow-lg rounded-lg p-4 sm:p-6 formBack">
        <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <h1 class="text-2xl font-bold text-gray-800">Sports List</h1>
            <div class="w-full md:w-auto flex flex-col sm:flex-row items-center gap-2">
                ...
                <input type="text" id="table-search" class="w-full sm:w-auto px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Search sports...">
                <a href="{{ url('sports/create') }}" class="w-full sm:w-auto bg-transparent text-black font-semibold py-2 px-4 border border-black rounded-lg hover:bg-black hover:text-white transition-colors text-center no-underline">Add New Sport</a>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full bg-transparent rounded-lg">
                ...
                <tbody id="sports-table-body" class="divide-y divide-gray-300">
...
<script>
    // --- Sports Data ---
    const sportsData = [
        { sl: 1, name: "Football", remark: "Team sport played with a ball between two teams of 11 players.", status: "Active" },
        { sl: 2, name: "Basketball", remark: "Team sport in which two teams, most commonly of five players each, opposing one another on a rectangular court.", status: "Active" },
        { sl: 3, name: "Cricket", remark: "Bat-and-ball game played between two teams of eleven players on a field at the centre of which is a 22-yard pitch.", status: "Active" },
        { sl: 4, name: "Hockey", remark: "Family of sports in which two teams play against each other by trying to maneuver a ball or a puck into the opponent's goal using a hockey stick.", status: "Inactive" },
        { sl: 5, name: "Tennis", remark: "Racket sport that can be played individually against a single opponent (singles) or between two teams of two players each (doubles).", status: "Active" }
    ];

    // --- Table Search and Pagination Logic ---
    const tableBody = document.getElementById('sports-table-body');
    const searchInput = document.getElementById('table-search');
    const paginationControls = document.getElementById('pagination-controls');
    const rowsPerPageSelect = document.getElementById('rows-per-page');
    
    let currentPage = 1;
    let rowsPerPage = rowsPerPageSelect.value === 'all' ? sportsData.length : parseInt(rowsPerPageSelect.value);
    ...
    function toggleStatus(sportId) {
        const sport = sportsData.find(item => item.sl === sportId);
        if (sport) {
            sport.status = sport.status === 'Active' ? 'Inactive' : 'Active';
        }
        const filteredSport = filteredData.find(item => item.sl === sportId);
        if (filteredSport) {
            filteredSport.status = sport.status;
        }
        displayTable(currentPage);
    }
    ...
    tableBody.addEventListener('click', (e) => {
        if (e.target.matches('span[data-id]')) {
            const sportId = parseInt(e.target.dataset.id);
            toggleStatus(sportId);
        }
    });
    
    searchInput.addEventListener('keyup', (e) => {
        const searchValue = e.target.value.toLowerCase();
        filteredData = sportsData.filter(item => {
    ...
    rowsPerPageSelect.addEventListener('change', (e) => {
        ...
        if (selectedValue === 'all') {
            rowsPerPage = filteredData.length > 0 ? filteredData.length : sportsData.length;
        } else {
