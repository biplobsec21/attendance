...
<div class="container mx-auto p-4">
    <div class="bg-white/30 shadow-lg rounded-lg p-6 formBack w-full max-w-lg mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-center text-gray-800">Create New Sport</h1>
        <form id="create-sport-form">
            <div class="mb-4">
                <label for="sport-name" class="block text-gray-700 text-sm font-bold mb-2">Sport Name <span class="text-red-600">*</span></label>
                <input type="text" id="sport-name" class="shadow-sm appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Enter sport name" required>
            </div>
            ...
            <div class="flex items-center justify-between mt-6">
                <a href="{{ url('sports/index') }}" class="bg-transparent text-black font-semibold py-2 px-4 border border-black rounded-lg hover:bg-black hover:text-white transition-colors no-underline">
                    Back to List
                </a>
...
@push('scripts')
<script>
    document.getElementById('create-sport-form').addEventListener('submit', function(e) {
        e.preventDefault();
        const sportName = document.getElementById('sport-name').value;
        const remark = document.getElementById('remark').value;
        const status = document.getElementById('status').value;
        console.log('Form Submitted', { sportName, remark, status });
