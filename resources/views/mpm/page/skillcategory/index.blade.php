...
@section('title', 'skillcategory List')
...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white/30 shadow-lg rounded-lg p-4 sm:p-6 formBack">
            <h1 class="text-2xl font-bold text-gray-800 mb-4 pb-2">skillcategory List</h1>

            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <form method="GET" action="{{ route('skillcategory.index') }}"
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        ...
                        placeholder="Search skillcategory...">
                    <select name="status"
                ...
                <div class="w-full md:w-auto md:ml-4">
                    <a href="{{ route('skillcategory.create') }}"
                ...
                <table class="min-w-full bg-transparent rounded-lg">
                    ...
                    <tbody class="divide-y divide-gray-300">
                        @forelse($skillCategories as $skillcategory)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $skillcategory->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $skillcategory->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form method="POST" action="{{ route('skillcategory.toggle-status', $skillcategory) }}"
                                        ...
                                        @method('PATCH')
                                        <button type="submit"
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $skillcategory->status_badge_class }} cursor-pointer border-0">
                                            {{ $skillcategory->status_text }}
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $skillcategory->created_at->format('M d, Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('skillcategory.show', $skillcategory) }}"
                                        class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                    <a href="{{ route('skillcategory.edit', $skillcategory) }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                    <form method="POST" action="{{ route('skillcategory.destroy', $skillcategory) }}"
                                        class="inline"
                                        onsubmit="return confirm('Are you sure you want to delete this skillcategory?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    @if (request()->filled('search'))
                                        No skillcategory found matching your criteria.
                                    @else
                                        No skillcategory available. <a href="{{ route('skillcategory.create') }}"
