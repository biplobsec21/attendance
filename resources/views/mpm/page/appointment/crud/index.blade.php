                    ...
                    <tbody class="divide-y divide-gray-300">
                        @forelse($appointments as $appointment)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $appointment->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form method="POST" action="{{ route('appointments.toggle-status', $appointment) }}"
                                        ...
                                        @method('PATCH')
                                        <button type="submit"
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $appointment->status_badge_class }} cursor-pointer border-0">
                                            {{ $appointment->status_text }}
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $appointment->created_at->format('M d, Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    ...
                                    <form method="POST" action="{{ route('appointments.destroy', $appointment) }}"
                                        class="inline"
                                        onsubmit="return confirm('Are you sure you want to delete this appointments?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    ...
                                    @else
                                        No appointments available. <a href="{{ route('appointments.create') }}"
