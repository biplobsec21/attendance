...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white/30 shadow-lg rounded-lg p-4 sm:p-6 formBack">
            <h1 class="text-2xl font-bold text-gray-800 mb-4 pb-2">Skill List</h1>

            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <form method="GET" action="{{ route('skill.index') }}" class="w-full md:max-w-2xl flex items-center gap-2">
                ...
                <table class="min-w-full bg-transparent rounded-lg">
                    ...
                    <tbody class="divide-y divide-gray-300">
                        @forelse($skills as $skill)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $skill->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $skill->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $skill->category->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form method="POST" action="{{ route('skill.toggle-status', $skill) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $skill->status_badge_class }} cursor-pointer border-0">
                                            {{ $skill->status_text }}
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $skill->created_at->format('M d, Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('skill.show', $skill) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    ...
                                    @else
                                        No skillcategory available. <a href="{{ route('skill.create') }}"
