...
@section('title', 'Medical Categories List')
...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white/30 shadow-lg rounded-lg p-4 sm:p-6 formBack">
            <h1 class="text-2xl font-bold text-gray-800 mb-4 pb-2">Medical Categories List</h1>

            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <div class="w-full flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <!-- Search Left -->
                    <form method="GET" action="{{ route('medical-categories.index') }}"
                    ...
                    <div class="w-full md:w-auto md:ml-4">
                        <a href="{{ route('medical-categories.create') }}"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-transparent rounded-lg">
                    <thead>
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ route('medical-categories.index', array_merge(request()->query(), ['sort_by' => 'id', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ route('medical-categories.index', array_merge(request()->query(), ['sort_by' => 'name', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc'])) }}"
                                    class="no-underline text-gray-500">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ route('medical-categories.index', array_merge(request()->query(), ['sort_by' => 'created_at', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc'])) }}"
                                    class="no-underline text-gray-500">
                                    Created At
                    ...
                    <tbody class="divide-y divide-gray-300">
                        @forelse($medicalCategories as $medicalCategory)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $medicalCategory->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap font-medium">{{ $medicalCategory->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $medicalCategory->created_at->format('M d, Y') }}
                                </td>
                                {{-- <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $medicalCategory->soldiers_count ?? $medicalCategory->soldiers()->count() }}
                                </td> --}}
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('medical-categories.show', $medicalCategory) }}"
                                        class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                    <a href="{{ route('medical-categories.edit', $medicalCategory) }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                    <form method="POST"
                                        action="{{ route('medical-categories.destroy', $medicalCategory) }}" class="inline"
                                        onsubmit="return confirm('Are you sure you want to delete this medical category?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                                    @if (request()->has('search'))
                                        No medical categories found matching your criteria.
                                    @else
                                        No medical categories available. <a href="{{ route('medical-categories.create') }}"
