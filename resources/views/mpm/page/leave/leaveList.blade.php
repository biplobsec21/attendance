...
@section('content')
    ...
    <div class="container mx-auto p-4">
        <div class="bg-white shadow-lg rounded-xl p-6 sm:p-8 formBack">
            ...
            <!-- Leave Table Container (will be updated via AJAX) -->
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-2xl p-6">
            ...
            <form id="leaveForm" action="{{ route('leave.leaveApplicationSubmit') }}" method="POST"
                ...
                <div>
                    ...
                    <!-- Selected Soldier Display -->
                    <div id="selectedSoldierDisplay" class="mb-2 p-3 bg-green-50 border border-green-200 rounded-lg hidden">
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-sm font-semibold text-green-800" id="selectedSoldierName"></p>
                                <p class="text-xs text-green-600" id="selectedSoldierDetails"></p>
                            </div>
                            <button type="button" id="changeSoldierBtn"
                    ...
                    <!-- Search Input and Dropdown (initially visible) -->
                    <div id="soldierSearchSection">
                        <!-- Search Input -->
                        <div class="relative mb-2">
                            <input type="text" id="soldierSearch" placeholder="Search by name or army number..."
                                class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-400 focus:outline-none">
                        ...
                        <div class="max-h-60 overflow-y-auto border rounded-lg">
                            <select name="soldier_id" id="soldierSelect" size="8"
                                class="w-full border-0 focus:ring-0 focus:outline-none soldier-dropdown" required>
                                <option value="">-- Select Profile --</option>
                                @foreach ($profiles as $profile)
                                    <option value="{{ $profile->id }}" data-name="{{ strtolower($profile->full_name) }}"
...
@push('styles')
    <style>
        ...
        .soldier-dropdown option {
            padding: 8px 12px;
            border-bottom: 1px solid #f3f4f6;
            cursor: pointer;
        }

        .soldier-dropdown option:hover {
            background-color: #fef3c7;
        }

        .soldier-dropdown option:checked {
            background-color: #fed7aa;
            color: #000;
        }

        .soldier-dropdown:focus {
            outline: none;
            ring: 0;
        }
...
@push('scripts')
    <script>
        ...
        function initializeEditButtons() {
            document.querySelectorAll('.editLeaveBtn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    document.querySelector('[name="soldier_id"]').value = soldierId;
                    ...
                    const selectedOption = soldierSelect.querySelector(`option[value="${soldierId}"]`);
                    if (selectedOption) {
                        showSelectedSoldier(selectedOption);
                    }
        ...
        const soldierSelect = document.getElementById('soldierSelect');
        const selectedSoldierDisplay = document.getElementById('selectedSoldierDisplay');
        const selectedSoldierName = document.getElementById('selectedSoldierName');
        const selectedSoldierDetails = document.getElementById('selectedSoldierDetails');
        const changeSoldierBtn = document.getElementById('changeSoldierBtn');
        const soldierSearchSection = document.getElementById('soldierSearchSection');

        // Soldier search functionality
        soldierSearch.addEventListener('input', function() {
            const searchTerm = this.value.toLowerCase().trim();
            const options = soldierSelect.querySelectorAll('option');
            ...
            options.forEach(option => {
                ...
                const soldierName = option.getAttribute('data-name') || '';
                const armyNo = option.getAttribute('data-army-no') || '';

                if (soldierName.includes(searchTerm) || armyNo.includes(searchTerm)) {
                    option.style.display = 'block';
                } else {
        ...
        // Show selected soldier in display box and hide search section
        function showSelectedSoldier(selectedOption) {
            if (selectedOption && selectedOption.value !== '') {
                const displayName = selectedOption.getAttribute('data-display-name');
                const armyNo = selectedOption.getAttribute('data-army-no');
                const rank = selectedOption.getAttribute('data-rank');

                selectedSoldierName.textContent = displayName;
                selectedSoldierDetails.textContent = `Army No: ${armyNo} | Rank: ${rank}`;
                selectedSoldierDisplay.classList.remove('hidden');
                soldierSearchSection.classList.add('hidden');
            }
        }

        // Show search section to change soldier
        changeSoldierBtn.addEventListener('click', function() {
            selectedSoldierDisplay.classList.add('hidden');
            soldierSearchSection.classList.remove('hidden');
            soldierSearch.value = '';
            soldierSelect.value = '';
        ...
        soldierSelect.addEventListener('change', function() {
