...
class StoreDutyRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'duty_name' => 'required|string|max:255|unique:duties,duty_name',
    ...
    public function withValidator(Validator $validator)
    {
        $validator->after(function (Validator $validator) {
            ...
            // Validate fixed soldiers availability
            if (!empty($data['fixed_soldiers'])) {
                $this->validateFixedSoldiers($validator, $data['fixed_soldiers']);
            }
    ...
    /**
     * Validate fixed soldiers availability and conflicts
     */
    private function validateFixedSoldiers(Validator $validator, array $fixedSoldiers): void
    {
        ...
        foreach ($fixedSoldiers as $index => $soldierData) {
            ...
            // Check for duplicate soldier assignments
            ...
            // Check soldier availability
            if (!$dutyService->isSoldierAvailableForDuty($soldier)) {
                $validator->errors()->add(
                    "fixed_soldiers.{$index}.soldier_id",
